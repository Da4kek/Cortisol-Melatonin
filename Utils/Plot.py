...
def fit_sin(tt, yy, plot=False):
    ...
    def sinfunc(t, A, w, p, c): return A * \
        numpy.sin((t/(freq))*2*numpy.pi + p) + c

    popt, pcov = scipy.optimize.curve_fit(sinfunc, tt, yy, p0=guess)
    A, w, p, c = popt
    f = w/(2.*numpy.pi)

    def fitfunc(t): return A * numpy.sin((t/freq)*2*numpy.pi + p) + c
    ...
    res = {"amp": abs(A), "phase": p, "APhase": acrophase_timestamp, "offset": c,
           "r2": coefficient_of_dermination, "tt": tt, 'ff': fitfunc(tt)}

    if plot == True:
        fig, ax = plt.subplots(figsize=(6, 5))
        ax.plot(tt, yy, "-k", linewidth=1, alpha=0.3)
        ax.plot(tt, fitfunc(tt), "r-", label="circadian rhythm", linewidth=1)
        plt.xticks(numpy.arange(130000)[::20000][2:], [
                   '11AM day 1', '5:30PM day 1', '10PM day 1', '3:30AM day 2', '9AM day 2'], rotation=90)
        plt.legend()
        fig.tight_layout()
        plt.show()
